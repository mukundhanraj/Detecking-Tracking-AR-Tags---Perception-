...
def g_corners(edges):
    ...
    corners = find_sq(points[:, 0], thresh=20)
    return corners
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            img = background_remove(gray)
            edges = edg_det(img)
            corners =g_corners(edges)
            ...
            try:
                h = homogrf(corners[:, 0], corners[:, 1], [
                                     0, 80, 80, 0], [0, 0, 80, 80])
                H = np.linalg.inv(h)
                tag = invwarping(gray, h, tag)

                _, ori = ar_tag(tag)
                #cv2.imshow('tag', tag)

                template = cv2.imread('testudo.png')
                template = cv2.resize(template, (80, 80))
                for i in range(ori):
                    template = cv2.rotate(
                        template, cv2.ROTATE_90_CLOCKWISE)

                dst = Warping(template, H, rframe)
